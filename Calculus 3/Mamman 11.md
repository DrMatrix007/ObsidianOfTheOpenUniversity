...
ניקח $S'=S \cup \{0^{[3]}\}$.
נניח כי $S'$ משטח.
לכן קיימת קבוצה  $\{U_i\}_{i \in I}$ כאשר:
  $$S' \subseteq \bigcup_{i \in I}U_i$$
   $$\forall i \in I (S \cap U_i \text{ are homeomorphic to } \varOmega \text{ open group in }\mathbb{R}^2 )$$
נקבל:
   $$\exists i \in I (0^{[3]} \in U_i)$$
ניקח $k$ כאשר  $0^{[3]} \in U_k$. ניקח $h$ פונ' הומיאומורפיזם על $\varOmega$
ניקח:
$$x=h^{-1}(0^{[3]})$$
מפני ש $h^{-1}(\varOmega)$ קבוצה פתוחה, קיים $r$ כאשר:
$$A = h^{-1}(\varOmega) \cap B(0^{[3]},r) \ne \emptyset$$
לפי סעיף ב, הקבוצה $A$ היא קשורה מסילתית.
ניקח:
$$B = h(A)$$
לכן מתקיים
לפי טענה 2.ט.2, הקבוצה $B$ קשורה מסילתית. בנוסף, $B$ היא גם קבוצה פתוחה.
מפני ש $B$ קבוצה פתוחה, אזי גם $B \backslash \{x\}$ גם קשורה מסילתית, היות ואפשר "לעקוף" את $x$ בסביבה של $x$ (כי $B$ פתוחה וידוע כי הקבוצה $B(0^{[3]},r)\backslash\{0\}$ קשורה מסילתית לכל $r$).
נסתכל על:
$$h(A \backslash \{x\}) = B \backslash \{x\}$$
(כי $h$ חח"ע) לכן:
$$h^{-1}(B \backslash \{x\}) = A \backslash \{x\}$$
ונקבל לפי טעמה 2.ט.2, כי $A\backslash \{x\}$ קשורה מסילתית, סתירה לסעיף ב.
לכן $S'$ לא משטח.