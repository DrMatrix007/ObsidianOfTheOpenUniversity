...
ולכן הקבוצה $S$ היא טלאי דו ממדי (כמובן ש $\mathbb{R}^2 \backslash \{(0,0,0)\}$ קבוצה פתוחה)
ולכן הקבוצה $S$ משטח
משל א
## סעיף ב
יהי $r \in (0,1]$
...
## סעיף ג
אוכיח טענת עזר:
יהי טלאי $S$. לכל קבוצה פתוחה $U$, הקבוצה $U \cap S$ היא טלאי.
נגדיר:
$$ h: \Omega \rightarrow S$$
כאשר $h$ הומיאומורפיזם על $S$, והקבוצה $\Omega$ פתוחה.
נניח בשלילה כי $h^{-1}(S \cap U)$ לא פתוחה.
לכן מתקיים:
$$\exists_{x \in h^{-1}(S \cap U)}(\exists_{(x_n)_n^\infty}(x_n \xrightarrow[n \rightarrow \infty]{}x \text{ and } \forall_n(x_n \notin h^{-1}(S \cap U))))$$
מהפתיחות של $\Omega$, נקבל $x_n \in \Omega$ כמעט לכל $n$. ללא הגבלת הכלליות, נניח ששייכות זאת מתקיימת לכל $n$.
ניקח סדרה $(y_n)_n^{\infty}$ כאשר מתקיים:
$$n = h(x_n) \in S$$
$$y = h(x)$$
וידוע כי מתקיים
$$y_n \notin S \cap U\ ,\ y \in S \cap U$$
נקבל
$$y_n \notin U\ ,\ y \in U$$
וזו סתירה כי $U$ קבוצה פתוחה.
לכן  $h^{-1}(S \cap U)$ פתוחה, ו $h$ היא הומיאומורפיזם מ $S \cap U$ ל  $h^{-1}(S \cap U)$ 
לכן, הקבוצה $U \cap S$ היא טלאי.
נגדיר $S' = S \cup \{0^{[3]}\}$
לפי הגדרת 
  
$$\exists i \in I (0^{[3]} \in U_i)$$
ניקח $k$ כאשר  $0^{[3]} \in U_k$. ניקח $h: \Omega \rightarrow U_k \cap S'$ פונ' הומיאומורפיזם על $\Omega$
ניקח:
$$x=h^{-1}(0^{[3]})$$
מפני שהקבוצה $U_k$ פתוחה, קיים $r$ כאשר:
$$A = h^{-1}(\Omega) \cap B(0^{[3]},r) \ne \emptyset$$
לפי הטענה, קבוצה זו היא טלאי.
לפי סעיף ב, הקבוצה $A$ היא קשורה מסילתית.
ניקח הומיאומורפיזם:
$$h':\Omega' \rightarrow A$$
ולכן מתקיים
$$ h'^{-1}: A \rightarrow \Omega'$$
הומיאומורפיזם, כלומר פונ זו היא פונ' חחע ורציפה.
לפי טענה 2.ט.2, הקבוצה $A$ קשורה מסילתית. נקבל כי $\Omega'$ קבוצה קשורה מסילתית
נסתכל על הקבוצה $\Omega' \backslash \{h'^{-1}(0)\}$.
קיים $r$ כאשר:
$$B(h'^{-1}(0);r) \subseteq \Omega'$$
יהי $a,b \in \Omega' \backslash \{h'^{-1}(0)\}$. 
ידוע כי קיימת מסילה $\varphi : [0,1] \rightarrow \Omega'$ ב $\Omega'$ מ$a$ ל$b$.
נבדיל בין 2 מקרים:
### מקרה 1 $h'^{-1}(0) \notin \varphi([0,1])$
לכן המסילה היא גם ב $\Omega' \backslash \{h'^{-1}(0)\}$ ולכן קיים מסילה
### מקרה 2 $h'^{-1}(0) \in \varphi([0,1])$
מפני ש $\Omega'$ היא קבוצה פתוחה, ידוע כי קיים:
$$B(h'^{-1}(0);r) \subseteq \Omega'$$
ניקח $B(0;\frac{r}{2})$. ניתן "להחליף" את הסביבה של $h'^{-1}(0)$ במסלול על $S(h'^{-1}(0);\frac{r}{2})$, היות והקבוצה הזו היא קשורה מסילתית. לכן קיים מסלול ב $\Omega \backslash \{h'^{-1}(0)\}$.

לכן הקבוצה הזו היא קשורה מסילתית.

ונקבל לפי טענה 2.ט.2, כי $A \backslash \{h'^{-1}(0)\}$ קשורה מסילתית, סתירה לסעיף ב.
לכן $S'$ לא משטח.
מש"ל ג.